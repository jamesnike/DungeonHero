...
>
> **删除** 将卡牌送入坟场（或按 destination 指定路由），**默认不触发**任何弃置副作用（onDiscardDraw / onDiscardDamage / 弹射护符 catapult / 弃牌生金 discard-profit / 等）。用于商店删牌、事件删牌、`净册涌泉`、`回炉重造` 等永久移除效果。
>
> **例外：** `招灵书印`（`delete-draw`）和 `雷霆符印`（`discard-zap`）这两张护符**显式响应「删除」**——它们的卡面文案明确包含「删除」语义。删除卡时，每张装备的 `招灵书印` 给左右装备栏 +1 临时攻击/+1 临时护甲、+2 金币；每张装备的 `雷霆符印` 对激活行随机怪物造成法术伤害。其它弃置联动**不**响应删除。
>
...
| 雷霆符印 | `discard-zap` | 弃牌**或删除**时对随机怪物造成伤害（删除路径见 §1） |
...
| 雷霆符印 (`discard-zap`) | `triggerDiscardShock()` 对随机怪物造成伤害（**也响应「删除」**——见 §1） |

> **规则：** 含"每"字样的触发器（弹射护符、雷霆符印、弃牌获利）一定在被弃牌自身效果之后结算。
> 例：弃掉一张 `onDiscardDraw: 2` 的牌 → 先抽 2 张（牌自身效果）→ 然后弹射护符再抽 2 张。
>
> **删除路径例外：** `雷霆符印` 和 `招灵书印` 也响应「删除」——见 §1 的弃置/删除区分表。删除路径不走 `APPLY_DISCARD_EFFECTS`，所以 `catapult` / `discard-profit` / `onDiscardDraw` / `onDiscardDamage` 等其它弃置联动**不**触发；只有这两张明确写了「删除」语义的护符响应。
...
| 雷霆符印 | 弃牌**或删除**时对随机怪物造成伤害（`max(0, 3 + permanentSpellDamageBonus)`）。删除路径见 §1。 |
