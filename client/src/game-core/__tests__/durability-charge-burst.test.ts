...
describe('蓄能裂击 (durability-charge-burst) 主效果', () => {
  ...
  it('dura 1 → 2 (no trigger): just +1 maxDurability +1 durability', () => {
    ...
    const wp = makeWeapon({ durability: 1, maxDurability: 5 });
    ...
    expect(finalEquip.maxDurability).toBe(6);
  ...
  it('dura 3 → 4 with 1 monster on board: trigger fires, monster takes 1-layer damage, dura ends at 2', () => {
    ...
    const wp = makeWeapon({ durability: 3, maxDurability: 5 });
  ...
  it('dura 5 → 6 (no trigger, only the value 4 triggers, not "max <= 4")', () => {
    const card = makeCard('5to6');
    const wp = makeWeapon({ durability: 5, maxDurability: 7 });
    const m = makeMonster('m1', 7);
    const state = makeState({
      handCards: [card],
      equipmentSlot1: wp,
    ...
    const finalEquip = result.state.equipmentSlot1 as EquipmentItem;
    expect(finalEquip.maxDurability).toBe(8);
    expect(finalEquip.durability).toBe(6);
    const finalMonster = result.state.activeCards.find(c => c?.id === 'm1') as any;
    expect(finalMonster.hp).toBe(7);
  });

  it('dura 4 (already 4) → +1 = 5 (no trigger, threshold is "post-add equals 4", not max)', () => {
    const card = makeCard('already-4');
    const wp = makeWeapon({ durability: 4, maxDurability: 6 });
    const m = makeMonster('m1', 7);
    const state = makeState({
      handCards: [card],
      equipmentSlot1: wp,
    ...
    const finalEquip = result.state.equipmentSlot1 as EquipmentItem;
    expect(finalEquip.maxDurability).toBe(7);
    expect(finalEquip.durability).toBe(5);
    const finalMonster = result.state.activeCards.find(c => c?.id === 'm1') as any;
    expect(finalMonster.hp).toBe(7);
  });
  ...
  it('echoMultiplier x2: dura 2 → first +1 = 3 (no trigger) → second +1 = 4 (trigger, dura→2)', () => {
    ...
    const wp = makeWeapon({ durability: 2, maxDurability: 6 });
    ...
    const finalEquip = result.state.equipmentSlot1 as EquipmentItem;
    expect(finalEquip.maxDurability).toBe(8); // 2 iterations of +1 each
  ...
  it('echoMultiplier x2: dura 3 → first +1 = 4 (trigger, dura→2) → second +1 = 3 (no trigger)', () => {
    ...
    const wp = makeWeapon({ durability: 3, maxDurability: 6 });
    const m = makeMonster('m1', 5);
    const state = makeState({
      handCards: [card],
      equipmentSlot1: wp,
  ...
  it('works on slot 2 (right slot) too — slot-routing not hardcoded to slot1', () => {
    ...
    const wp = makeWeapon({ id: 'w2', durability: 3, maxDurability: 5 });
    const m = makeMonster('m1', 5);
    const state = makeState({
      handCards: [card],
      equipmentSlot1: null,
